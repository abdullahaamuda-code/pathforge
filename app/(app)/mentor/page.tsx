...
export default function MentorPage() {
  ...
  async function initMentor() {
    try {
      ...
      if (savedChat) {
        ...
        const greeting = {
          role: "assistant",
          content: `Hey ${firstName} — I'm your PathForge mentor. You're a ${stage} interested in ${interest}. What's on your mind?`,
        };
        history = [greeting];
        setMessages(history);
  ...
  async function sendMessageWithHistory(message, history, userData) {
    const chatKey = `pathforge_chat_${user.uid}`;
    ...
    <div className="min-h-screen bg-[#080a0f] text-white flex flex-col">
      ...
      <div className="border-b border-[#2C2C2A] px-6 py-4 flex items-center justify-between sticky top-0 bg-[#080a0f] z-10">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <Link href="/mentor" className="text-white text-xs font-medium">AI Mentor</Link>
        </div>
      </div>
          ...
          {[
            ...
            <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
               stroke={item.href === "/mentor" ? "#7F77DD" : "#888780"}
                strokeLinecap="round" strokeLinejoin="round">
                {item.icon}
              </svg>
              <span className={`text-[9px] ${item.href === "/mentor" ? "text-[#7F77DD]" : "text-[#444441]"}`}>
                {item.label}
              </span>
            </Link>
          ))}
