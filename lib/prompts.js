export function buildRoadmapPrompt(user) {
  ...
  const languageInstruction = user.language === "fr"
    ? "Generate ALL text content in French. Every field, every sentence, every word must be in French. This is critical."
  ...
  return {
    ...
    user: `${languageInstruction}

Generate a personalized career roadmap for someone who is ${stage} from ${country}.
...
Return a JSON object with this exact structure:
{
  "careerPaths": [
    {
      ...
      "matchScore": 92,
      "whyItFits": "1 sentence starting with 'Your interest in...' — specific to their actual listed interests",
    ...
    "steps": [
      {
        ...
        "actions": [
          "Specific action naming exact resource",
  ...
  "quickWins": [
    "One specific thing you can do today or this week",
    "Second quick win — specific and actionable",
    "Third quick win — specific and actionable"
  ],
  "motivationalNote": "2-3 sentences written directly to this person using 'you'. Must reference their country, their specific interests, and their current stage."
}
...
Return:
{
  "score": 85,
  "label": "Strong Match",
  "reason": "One specific sentence written in second person directly to the user — say 'your' and 'you', never say 'the student' or 'their'."
}

Score 0-100. Label must be exactly one of: "Strong Match" (80+), "Good Match" (60-79), "Possible Match" (40-59), "Low Match" (below 40). Reason must use second person — you/your only.`,
  };
}
...
export function buildMentorPrompt(user) {
  ...
  const languageInstruction = user.language === "fr"
    ? "CRITICAL: You MUST always respond in French. Every single message must be in French. Never switch to English."
...
- If they say "hi", "hey", "bonjour" or any greeting just respond warmly and naturally like a real person would
...
- Use emojis occasionally and naturally — not on every message, only when it fits the tone. A thumbs up, a fire, a sparkle here and there. Never overdo it.`;
}
