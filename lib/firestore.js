...
// Save monthly job cache per user
export async function saveUserJobs(uid, jobs) {
  await setDoc(doc(db, "userJobs", uid), {
    uid,
    jobs,
    cachedAt: serverTimestamp(),
  });
...
// Get monthly job cache — only expires after 30 days
export async function getUserJobs(uid) {
  try {
    const snap = await getDoc(doc(db, "userJobs", uid));
    if (!snap.exists()) return null;
    const data = snap.data();
    const cachedAt = data.cachedAt?.toDate();
    if (!cachedAt) return null;
